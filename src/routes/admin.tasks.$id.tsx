  ...
  return (
    <PageCard title="View Task">
      <div className="grid md:grid-cols-2 gap-4 max-w-3xl">
        <div className="md:col-span-2"><Label>Subject</Label><Input defaultValue={task.subject} readOnly /></div>
        <div><Label>Start Date</Label><Input defaultValue={task.startDate} readOnly /></div>
        <div><Label>Resource</Label><Input defaultValue={task.resourceName} readOnly /></div>
        <div><Label>Project</Label><Input defaultValue={task.project} readOnly /></div>
        <div className="md:col-span-2"><Label>Task Notes (Append on top, do not overwrite)</Label><Textarea rows={6} value={notes} onChange={(e) => setNotes(e.target.value)} /></div>
        <div>
        ...
        <div className="md:col-span-2 flex gap-2">
          <Button onClick={() => { update(id, { status, notes }); router.navigate({ to: "/admin/tasks" }); }}>Update</Button>
